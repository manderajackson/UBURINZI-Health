...
**Chronic patient monitoring between clinic visits — SMS-first, Africa-ready, built inside a Kigali clinic.**
...
| Email | Password | Role |
|---|---|---|
| `founder@uburinzi.rw` | `uburinzi2026` | admin |
| `nurse@uburinzi.rw` | `uburinzi2026` | clinician |
...
│   ├── cli.py            init-db / seed / reset / send-due / import-csv / stats
│   ├── charts.py         dependency-free SVG charts (no CDN, works offline)
│   ├── routes_ui.py      dashboard, patients, alerts, messages, protocols, settings
│   ├── routes_api.py     webhooks, JSON API, CSV export, EMR integration
│   └── messaging/
...
├── tests/                39 pytest tests (SMS + WhatsApp + voice + escalation)
...
## 10. Governance, consent and data protection
...
## 12. Why this is Pan-African by design
...
Built in Kigali for the **timbuktoo HealthTech LaunchPad** application.
