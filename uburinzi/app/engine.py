...
layer described in the timbuktoo application.
"""
    ...
    if isinstance(parsed, ReplyChoice):
        if parsed is ReplyChoice.YES:
            ...
            open_alert(
                ...
                message=f"{patient.full_name} reported not feeling well by SMS. Call back today.",
            )
...
def place_voice_call(session, patient, purpose: str = "checkin", program: str = "common") -> VoiceCall:
    ...
    msg = _queue(
        ...
        channel=Channel.VOICE.value,
        dedupe_key=f"voice:{patient.id}:{now().date().isoformat()}:{purpose}",
    )
    call = VoiceCall(
        patient_id=patient.id,
        outbound_id=msg.id,
...
def run_escalations(session) -> dict:
    ...
    session.commit()
    return {"whatsapp_sent": whatsapp_sent, "voice_calls": calls_placed, "patients_considered": len(patients)}
