...
def check_balance(settings: dict, timeout: float = 15.0) -> dict:
    ...
    # Same throttle handling as _post: AT answers a rate-limited call with a fake
    # 401, and "Test connection" is usually clicked right after something else.
    for attempt in range(3):
        try:
            ...
            throttled = (
                ...
                and "authentication is invalid" in (resp.text or "").lower()
                and attempt < 2
            )
            if throttled:
                log.warning("AT throttled the balance check — retrying in %ss", (attempt + 1) * 4)
            ...
            time.sleep((attempt + 1) * 4)
    return {"ok": False, "error": "balance check failed after retries"}


def _post(url: str, *, settings: dict, data: dict | None = None, json_body: dict | None = None,
          timeout: float = 20.0) -> tuple[int, dict, str]:
    """POST to Africa's Talking, absorbing their throttle.

    AT rate-limits bursts and answers a throttled request with HTTP 401
    "The supplied authentication is invalid" — the same message a genuinely bad
    key produces. Because a burst of check-ins is exactly what this app does, we
    retry a 401 a couple of times with a short backoff; a real credential error
    simply fails on the last attempt, so nothing is masked.
    """
    headers = _at_headers(settings)
    last = (401, {}, "The supplied authentication is invalid")
    for attempt in range(3):
        hdrs = dict(headers)
        try:
            if json_body is not None:
                hdrs["Content-Type"] = "application/json"
                resp = httpx.post(url, headers=hdrs, json=json_body, timeout=timeout)
            else:
                hdrs["Content-Type"] = "application/x-www-form-urlencoded"
                resp = httpx.post(url, headers=hdrs, data=data, timeout=timeout)
        except httpx.HTTPError as exc:
            # AT publishes no reachable voice endpoint in sandbox
            # (voice.sandbox.africastalking.com does not resolve), so a DNS
            # failure there means "you are on sandbox; calls need live".
            if "/call" in url and "sandbox" in url:
                return (0, {}, "Africa's Talking has no working voice endpoint on the sandbox "
                               "app — calls need a live app key, live username and an AT voice number")
            return (0, {}, f"Could not reach Africa's Talking at {url}: {type(exc).__name__}")
        try:
            payload = resp.json()
        except Exception:
            payload = {}
        last = (resp.status_code, payload, resp.text)
        throttled = (
            resp.status_code == 401
            and "authentication is invalid" in (resp.text or "").lower()
            and attempt < 2
        ...
        if not throttled:
            return last
        log.warning("AT throttled the request (401) — retrying in %ss", (attempt + 1) * 4)
        time.sleep((attempt + 1) * 4)
    return last
...
class AfricaTalkingSmsDriver(BaseDriver):
    ...
    def send(self, msg) -> SendResult:
        ...
        try:
            ...
            if code == 0:
                return SendResult(False, status="failed", error=text[:200] or "Could not reach Africa's Talking")
            ...
            # AT returns e.g. "RWF 12.0000" -- take the number, not the currency
            match = re.search(r"([0-9]+(?:\.[0-9]+)?)", str(first.get("cost", "")).replace(",", ""))
