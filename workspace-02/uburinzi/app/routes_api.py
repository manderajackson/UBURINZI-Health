...
def assetlinks(db: Session = Depends(get_db)):
    """Digital Asset Links — proves to Android that our site owns the TWA app.

    Without this file a Play Store app built with Bubblewrap opens with a browser
    address bar instead of running full-screen. Paste your Play signing
    certificate's SHA-256 into Channels (or set UBURINZI_PLAY_SHA256) and this
    endpoint serves the exact JSON Google expects.
    """
    package = settings_store.get(db, "play_package_name", "rw.uburinzi.health") or "rw.uburinzi.health"
    fingerprints = [
        f.strip()
        for f in (settings_store.get(db, "play_signing_sha256", "") or "").split(",")
        if f.strip()
    ]
    statement = [
        {
    ...
    return JSONResponse(statement, headers={"Cache-Control": "public, max-age=600"})


@router.get("/api/cron/scheduler")
@router.post("/api/cron/scheduler")
async def cron_scheduler(request: Request, daily: bool = False, db: Session = Depends(get_db)):
    """Replacement for the background thread on serverless hosts.

    Vercel Cron (or any external scheduler: cron-job.org, GitHub Actions) calls
    this on a schedule. Locally you never need it — uvicorn runs the background
    worker itself.

    Secured with CRON_SECRET: set that variable and Vercel sends
    ...
    if secret:
        ...
        if header.replace("Bearer ", "").strip() != secret:
            raise HTTPException(401, "invalid cron secret")

    from app.engine import run_daily_jobs, run_escalations, run_scheduler

    payload: dict = {"ran_at": now().isoformat()}
    scheduler = run_scheduler(db, force=True)
    payload["scheduler"] = scheduler
    if daily or datetime.now().hour >= 7:
        payload["daily"] = run_daily_jobs(db)
    payload["escalations"] = run_escalations(db)
    return {"ok": True, **payload}


@router.post("/api/webhooks/ussd")
async def ussd_inbound(request: Request, db: Session = Depends(get_db)):
    """Africa's Talking USSD callback.

    AT posts `sessionId`, `serviceCode`, `phoneNumber`, `text` (form-encoded) and
    expects a plain-text body beginning with `CON ` (keep the session open) or
    `END ` (finish it). This is the reply path in Rwanda, where patients cannot
    answer an alphanumeric sender ID.
    """
    from app.ussd import handle_ussd

    form = dict(await request.form()) if not request.headers.get(
        "content-type", ""
    ).startswith("application/json") else await request.json()

    body = handle_ussd(
        db,
        session_id=str(form.get("sessionId") or ""),
        phone=str(form.get("phoneNumber") or ""),
