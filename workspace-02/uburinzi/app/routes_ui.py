...
def login_page(request: Request, db: Session = Depends(get_db)):
    return render(request, db, "login.html", active="login", error=None, demo_mode=config.DEMO_MODE)
...
def login(
    ...
    if not user or not verify_password(password, user.password_hash):
        return render(
            request, db, "login.html", active="login",
            error="Wrong email or password.", demo_mode=config.DEMO_MODE,
        )
...
def privacy(request: Request, db: Session = Depends(get_db)):
    """Public privacy policy — required by the Google Play listing."""
    from datetime import date

    from app import settings_store

    email = settings_store.get(db, "privacy_contact_email", "") or "privacy@uburinzi.rw"
    return render(request, db, "privacy.html", active="privacy", today=date.today().isoformat(), contact_email=email)
...
@router.post("/channels/reply-path")
async def channels_reply_path(request: Request, db: Session = Depends(get_db), user: User = Depends(login_required)):
    """Which channel carries the patient's answer.

    Rwanda (and Uganda, Bangladesh, Thailand, Vietnam, Japan) cannot receive
    replies to an A2P SMS, so "Reply 1=Yes" is a dead end there -- USSD or a
    short code has to carry the response instead.
    """
    form = await request.form()
    channel = str(form.get("reply_channel") or "sms").strip()
    if channel not in ("sms", "ussd", "shortcode"):
        channel = "sms"
    settings_store.set_value(db, "reply_channel", channel)
    settings_store.set_value(db, "ussd_code", str(form.get("ussd_code") or "").strip())
    db.add(AuditLog(actor=user.email, action="channels.reply_path", detail=f"reply_channel={channel}"))
    db.commit()
    return flash_redirect("/channels", f"Reply path saved ({channel}).", "ok")
